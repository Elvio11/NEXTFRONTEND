...
logger = logging.getLogger("server3_startup")
...
logger.info("[BOOT] Env defaults set.")
...
try:
    ...
    logger.info("[BOOT] scraper router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: scraper — {e}")
    raise

try:
    from routers import tailor

    logger.info("[BOOT] tailor router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: tailor — {e}")
    raise

try:
    from routers import cover_letter

    logger.info("[BOOT] cover_letter router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: cover_letter — {e}")
    raise

try:
    from routers import apply

    logger.info("[BOOT] apply router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: apply — {e}")
    raise

try:
    from routers import anti_ban

    logger.info("[BOOT] anti_ban router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: anti_ban — {e}")
    raise

try:
    from routers import followup

    logger.info("[BOOT] followup router OK")
except Exception as e:
    logger.error(f"[BOOT] FAILED: followup — {e}")
    raise

logger.info("[BOOT] All routers loaded. Starting uvicorn...")
...
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    logger.error(
        f"[server3] Unhandled exception: {type(exc).__name__}: {str(exc)[:200]}\n{tb}"
    )
    return JSONResponse(
        status_code=500,
        content={
