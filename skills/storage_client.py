...
async def put(key: str, data: bytes) -> None:
    ...
    await loop.run_in_executor(
        None,
        lambda: get_s3_client().put_object(Bucket=BUCKET(), Key=key, Body=data)
    )
...
async def get(key: str) -> bytes:
    ...
    try:
        response = await loop.run_in_executor(
            None,
            lambda: get_s3_client().get_object(Bucket=BUCKET(), Key=key)
        )
...
async def delete(key: str) -> None:
    ...
    try:
        await loop.run_in_executor(
            None,
            lambda: get_s3_client().delete_object(Bucket=BUCKET(), Key=key)
        )
    except ClientError:
...
async def exists(key: str) -> bool:
    ...
    try:
        await loop.run_in_executor(
            None,
            lambda: get_s3_client().head_object(Bucket=BUCKET(), Key=key)
        )
        return True
    except ClientError:
        return False
...
async def list_keys(prefix: str) -> list[str]:
    ...
    response = await loop.run_in_executor(
        None,
        lambda: get_s3_client().list_objects_v2(Bucket=BUCKET(), Prefix=prefix)
    )
