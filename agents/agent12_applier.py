...
def _is_in_review_mode(auto_apply_activated_at) -> bool:
    ...
    if isinstance(auto_apply_activated_at, str):
        activated = datetime.fromisoformat(
            auto_apply_activated_at.replace("Z", "+00:00")
        )
    else:
...
async def _call_anti_ban(user_id: str, action_type: str, platform: str) -> dict:
    ...
    try:
        ...
        return {
            "risk_level": "critical",
            "proceed": False,
            "reason": f"anti_ban_unreachable: {exc}",
        }
...
async def run_applier(
    ...
    try:
        ...
        if subscription_tier == "free":
            await log_skip(run_id, "free_tier_no_applies")
            return {
                "status": "skipped",
                "applied": 0,
                "failed": 0,
                "reason": "free_tier_no_applies",
                "applications": [],
            }

        # Student tier: only URL redirect applies (tier 2), max 3/day, 30/month
        if subscription_tier == "student":
            if apply_tier != 2:
                await log_skip(run_id, "student_tier_requires_url_redirect")
                return {
                    "status": "skipped",
                    "applied": 0,
                    "failed": 0,
                    "reason": "student_tier_requires_url_redirect",
                    "applications": [],
                }

            # Check student-specific caps
            daily_limit = 3
            monthly_limit = 30
            ...
            if monthly_count >= monthly_limit:
                await log_skip(
                    run_id, f"monthly_cap_hit:{monthly_count}/{monthly_limit}"
                )
                return {
                    "status": "skipped",
                    "applied": 0,
        ...
        # Professional tier: only auto-apply (tier 1), max 10/day, 250/month
        if subscription_tier == "professional":
            if apply_tier != 1:
                await log_skip(run_id, "professional_tier_requires_auto_apply")
                return {
                    "status": "skipped",
                    "applied": 0,
                    "failed": 0,
                    "reason": "professional_tier_requires_auto_apply",
                    "applications": [],
                }
        ...
        remaining_today = min(
            daily_limit - daily_count, 250 - monthly_count, max_applies
        )
        ...
        for score_row in fit_scores_result.data:
            ...
            job = job_result.data
            company_lower = (
                job.get("company_canonical") or job.get("company", "")
            ).lower()
                ...
                applied_jobs.append(
                    {
                        ...
                        "site": platform,
                        "status": "skipped",
                        "reason": f"anti_ban:{anti_ban_result.get('risk_level')}",
                    }
                )
                continue
