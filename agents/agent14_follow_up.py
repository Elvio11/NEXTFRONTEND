...
REJECTION_KEYWORDS = [
    ...
    "other candidates", "position filled", "decided to move forward with other",
]

INTERVIEW_KEYWORDS = [
    "interview", "call scheduled", "meet with", "video call", "phone screen",
    "we'd like to", "invitation to interview", "next steps", "availability",
]
...
async def _generate_followup_email(stage: int, job_title: str, company: str, user_name: str) -> str:
...
async def _send_gmail(access_token: str, to: str, subject: str, body: str, thread_id: Optional[str] = None) -> None:
...
async def _detect_interview_and_notify(user_id: str, app_id: str, job_title: str, company: str, access_token: str):
        ...
        for m in messages:
            ...
            if any(kw in snippet for kw in INTERVIEW_KEYWORDS):
                ...
                await client.post(
                    ...
                    json={"type": "interview_detected", "app_id": app_id, "user_id": user_id},
...
async def run_follow_up() -> dict:
    ...
    try:
        ...
        for app in (apps.data or []):
            try:
                user_id = app["user_id"]
                access_token = await _refresh_google_token(user_id)
                
                # Check for replies/rejections (Interview detection run here)
                await _detect_interview_and_notify(user_id, app["id"], app["job_title"], app["company_name"], access_token)
                
                # Logic for sending next email sequence
                stage = app["follow_up_stage"]
                email_body = await _generate_followup_email(stage, app["job_title"], app["company_name"], app["user_full_name"])
                
                if email_body:
                    await _send_gmail(access_token, app["recruiter_email"] or "hr@company.com", f"Follow up: {app['job_title']} application", email_body)
        ...
        return {"status": "success", "duration_ms": duration_ms, "records_processed": records_processed, "error": None}
