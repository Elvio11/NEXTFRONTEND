...
- **Server 2 (Intelligence)**: Python 3.11 + FastAPI + CrewAI | URL: `http://localhost:8080` (Internal)
- **Server 3 (Execution)**: Python 3.11 + FastAPI + Selenium | URL: `http://localhost:8080` (Internal)
...
| 9 | Job Scraper | `server3/agents/agent9_scraper.py` | Complete | Multi-platform scraping via **Firecrawl MCP** + Free APIs. |
...
| 12 | Auto-Applier | MCP Tool (Playwright) | COMPLETE | V4: Integrated **Reactive Form Q&A** via Sarvam-M + Playwright MCP. |
| 13 | Anti-Ban/Form Q&A| MCP Tool (Adaptive) | COMPLETE | Integrated into **Playwright MCP's adaptive delay logic**. |
| 14 | Follow-Up Sender | `server3/agents/agent14_follow_up.py` | Complete | Context-aware follow-ups via **Official Gmail/Calendar MCP**. |
...
### Server 3
- `POST /api/agents/scraper` (MCP-driven)
- `POST /api/agents/tailor`
- `POST /api/agents/apply` (MCP-driven)
...
## Current Phase
- **Phase 6.1 (Production Hardening)** — [PHASES.md](file:///C:/Users/DELL/.gemini/antigravity/brain/0f4c5706-ea3e-493f-82f7-738250ff6639/PHASES.md) identifies this as the final V4 stabilization before Phase 7 Swarm.
- **Up Next**: Phase 7 (Advanced Humanizer & Multi-Agent Swarm Calibration).

## Last Worked On
- **V4 Architecture Migration**: Consolidated scraping into Firecrawl MCP.
- **MCP Migration**: Standardized all agents to use Official MCP servers (Playwright, Gmail, Google Calendar).
- **Reactive Q&A**: Implemented LLM-driven form answering for job applications.

## Open TODOs
- Server 1: Implement outbound Baileys send (deferred to Phase 3).
- Verification: End-to-end stress test of Firecrawl scraper concurrency.
