...
- **Server 1** (Gateway): Node.js 20 + Express + Nginx + Baileys + Telegram (grammY). Public-facing only. No agent logic lives here. Delegates everything to Servers 2 and 3 via HTTP POST.
...
## Storage — MinIO (S3-Compatible) Only

- **Supabase Storage and FluxShare are NOT used.** All file storage is via MinIO (S3-compatible) using `storage_client.py`.
- Doppler secrets: `S4_URL`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_BUCKET`.
- Storage keys follow the structure: `parsed-resumes/{user_id}.json.gz`, `skill-gaps/{user_id}.json.gz`, `jds/{fingerprint}.txt`, `tailored/{user_id}/{app_id}.pdf`, `screenshots/{agent}/{timestamp}.png`, etc.
- All 3 servers use the same S3 bucket. Direct filesystem writes to `/storage/` are deprecated.
...
- Current phase: **Phase 5.3 (Infrastructure Alignment)** — Aligning with March 16 Locked Decisions (MinIO, pgvector, new Tiers).
- Each phase must be fully deployed and tested before the next begins.
- Phase 6 is next: Build out Agent 9 with new scrapers and India remote filter.
