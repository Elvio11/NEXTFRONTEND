...
- **resumes** — Metadata only. Parsed JSON lives on MinIO. SHA-256 dedup via `file_hash`. `resume_embedding` stored for hybrid search.

### Job Pool (4 tables)
- **jobs** — 150K–200K active jobs. `fingerprint` (SHA-256) ensures dedup across platforms. `is_new` flag drives scoring efficiency. `pool_tier` and `remote_viability_score` drive matching. `jd_embedding` used for semantic search.
...
- **skill_gap_results** — One row per user. `top_gaps` JSONB (top 3 in DB). Full report on MinIO.
...
| Mode | Trigger | Scope |
|---|---|---|
| Full Scan | `fit_scores_stale = TRUE` (new user, new resume, weekly reset) | All `is_active = TRUE` jobs after pgvector hybrid search prefilter |
| Delta Only | Normal nightly run | Only `is_new = TRUE` jobs from latest scrape_run |

## Storage (MinIO S3 Keys)

| Content | Key Pattern | TTL |
|---|---|---|
| Parsed resume JSON | `parsed-resumes/{user_id}.json.gz` | Permanent |
| Raw JD text | `jds/{fingerprint}.txt` | 30 days |
| Skill gap report | `skill-gaps/{user_id}.json.gz` | Overwritten |
| Career intel | `career-intel/{user_id}.json.gz` | Overwritten |
| Tailored resume | `tailored/{user_id}/{app_id}.pdf` | 7 days |
| Cover letter | `cover-letters/{user_id}/{app_id}.txt` | 7 days |
| Apply screenshot | `screenshots/{agent}/{timestamp}.png` | 7 days |
| Live weights | `model-data/weights_current.json` | Permanent |
| Calibration report | `calibration/{run_id}.json.gz` | 90 days |
