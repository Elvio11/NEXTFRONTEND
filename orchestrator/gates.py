...
def gate_safety(trigger: str, user_id: str | None = None) -> None:
    ...
    if _trigger_involves_linkedin(trigger):
        ...
        limit = data.get(
            "linkedin_daily_limit", 2000
        )  # Default to 2000 if column not yet populated
...
def gate_account(user: dict, trigger: str) -> None:
    ...
    if _trigger_requires_session(trigger):
        ...
        if consecutive >= 3:
            get_supabase().table("users").update({"auto_apply_paused": True}).eq(
                "id", user_id
            ).execute()
            raise GateFailure(
                gate="account",
    ...
    if trigger == "apply_window_open":
        ...
        if tier != "professional":
            raise GateFailure(
                gate="account",
                message="Only professional users get auto-apply",
